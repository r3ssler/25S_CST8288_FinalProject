...
public class MaintenanceManagementServlet extends HttpServlet {
    ...
    /**
     * Updates the status of a vehicle manually.
     *
     * @param req  the HTTP request
     * @param resp the HTTP response
     */
    private void updateStatus(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        List<VehicleDTO> vehicles = vehicleService.search(vehicleNumber);
        if (!vehicles.isEmpty()) {
            VehicleDTO vehicle = vehicles.get(0);
            vehicle.setStatus(newStatus);
            vehicleService.update(vehicle); // Make sure VehicleService has update() method
        }

        // Redirect back to the component page after update
        resp.sendRedirect(req.getContextPath() + "/maintenance/components?vehicleNumber=" + vehicleNumber);
    }
